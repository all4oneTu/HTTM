...
def root():
    ...
    session_client = dialogflow_.SessionsClient()
    session = session_client.session_path("chatbot-xryq", mobnum)
    text_input = dialogflow_.types.TextInput(text=message, language_code='en-US')
    query_input  = dialogflow_.types.QueryInput(text=text_input)
